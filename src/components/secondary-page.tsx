...
export function PageHero({ eyebrow, title, copy, image }: { eyebrow: string; title: string; copy: string; image?: string }) {
  return <section className="relative flex min-h-[500px] items-end overflow-hidden bg-ink pb-16 pt-32 text-white"><Image src={image ?? pageImages.experiences} alt="South African landscape" fill priority sizes="100vw" className="object-cover" /><div className="absolute inset-0 bg-ink/65" /><Navbar /><div className="relative mx-auto w-full max-w-[1320px] px-5 lg:px-8"><p className="eyebrow text-gold">{eyebrow}</p><h1 className="mt-4 max-w-3xl font-serif text-6xl leading-[.9] md:text-8xl">{title}</h1><p className="mt-6 max-w-xl text-base leading-relaxed text-white/70">{copy}</p></div></section>;
}
...
export function MarketplacePage({ kind }: { kind: "guides" | "experiences" | "destinations" }) {
  ...
  return <main><PageHero eyebrow={config[0]} title={config[1]} copy={config[2]} image={pageImages[kind]} /><section className="bg-cream px-5 py-20 lg:px-8"><div className="mx-auto max-w-[1320px]">{kind !== "destinations" ? <MarketplaceResults kind={kind} /> : <div className="grid gap-4 md:grid-cols-3">{destinations.map((item) => <Link href={`/destinations/${item.slug}`} key={item.slug} className="group relative block aspect-[.82] overflow-hidden rounded-[4px]"><Image src={item.heroImage} alt={item.name} fill sizes="33vw" className="object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-ink/90 to-transparent" /><div className="absolute bottom-0 p-6 text-white"><p className="eyebrow text-gold">{item.province}</p><h2 className="mt-2 font-serif text-3xl">{item.name}</h2><p className="mt-2 text-xs text-white/70">{item.description}</p></div></Link>)}</div>}</div></section><Footer /></main>;
}
